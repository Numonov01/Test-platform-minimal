...
export const _userList = [...Array(20)].map((_, index) => ({
  ...
  height: _mock.number.nativeS(index),
  weight: _mock.number.nativeM(index),
  armLength: _mock.number.nativeS(index),
  legLength: _mock.number.nativeM(index),
  gender: (index % 2 ? 'male' : 'female') as 'male' | 'female',
  status:

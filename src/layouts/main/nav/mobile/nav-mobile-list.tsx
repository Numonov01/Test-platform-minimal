...
export function NavList({ data }: NavListProps) {
  const isHashLink = data.path.startsWith('#');
  ...
  const activeLinkResult = useActiveLink(data.path, !!data.children);

  const active = isHashLink ? activeSection === data.path : activeLinkResult;
